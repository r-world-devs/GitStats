  ...
  if (inherits(responses_list, "graphql_error")) {
    ...
    if (inherits(responses_list, "graphql_no_fields_error")) {
      if (verbose) {
        ...
        cli::cli_alert_danger("Your GraphQL schema does not recognize [{error_fields}] field{?s}.")
    ...
    if (check && verbose) {
      ...
      cli::cli_alert_danger("Your GraphQL does not recognize [{error_fields}] field{?s}.")
      cli::cli_alert_danger("Check version of your GitLab.")
    }
  }
