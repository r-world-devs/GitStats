# Authors and Citation

## Authors

- **Maciej Banas**. Author, maintainer.

- **Kamil Koziej**. Author.

- **Karolina Marcinkowska**. Author.

- **Matt Secrest**. Author.

## Citation

Source:
[`DESCRIPTION`](https://github.com/r-world-devs/GitStats/blob/v2.5.1.9001/DESCRIPTION)

Banas M, Koziej K, Marcinkowska K, Secrest M (2026). *GitStats:
Standardized Git Repository Data*. R package version 2.5.1.9001,
<https://r-world-devs.github.io/GitStats/>.

    @Manual{,
      title = {GitStats: Standardized Git Repository Data},
      author = {Maciej Banas and Kamil Koziej and Karolina Marcinkowska and Matt Secrest},
      year = {2026},
      note = {R package version 2.5.1.9001},
      url = {https://r-world-devs.github.io/GitStats/},
    }
